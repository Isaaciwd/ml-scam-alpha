#%%
"""Non-normalized variant of the latent global atmospheric mixer.

This implementation mirrors ``LatentGlobalAtmosMixer`` but intentionally
omits layer/batch normalization across the encoder, mixer, and decoder so
that absolute variable magnitudes are preserved. That assumption is crucial
for physical tasks where the scale of temperature, wind, or humidity carries
meaning and should not be rescaled implicitly by the model.
"""
...
class NNorm_LatentGlobalAtmosMixer(nn.Module):
    """Latent mixer identical to the normalized variant but without norm layers.

    This implementation mirrors ``LatentGlobalAtmosMixer`` and intentionally
    omits layer/batch normalization across the encoder, mixer, and decoder so
    that absolute variable magnitudes are preserved. That assumption is crucial
    for physical tasks where the scale of temperature, wind, or humidity carries
    meaning and should not be rescaled implicitly by the model.
...
if __name__ == '__main__':
    ...
    model = NNorm_LatentGlobalAtmosMixer(
